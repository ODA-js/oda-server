import {
  ...
  IPackage,
  IRelation,
  IValidationResultInput,
  IValidationResultModel,
} from 'oda-model';
import {
  EntityContext,
  EntityLevel,
...
export class ModelVisitor {
  ...
  public visit(model: IModel): IValidationResultModel[] {
    const context = new ModelContext(model);
    const result: IValidationResultModel[] = [];
    if (context.isValid) {
      let done = false;
      while (!done) {
        try {
